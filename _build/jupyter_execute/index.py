...
template_p_whiten = util.whiten(template_p,psd_H1,dt)
template_c_whiten = util.whiten(template_c,psd_H1,dt)
...
for det in dets:
    ...
    template_whitened = util.whiten(template_rolled,interp1d(freqs, data_psd),dt)  # whiten the template
...
# write the files:
util.write_wavfile(eventname+"_H1_whitenbp.wav",int(fs), strain_H1_whitenbp[indxd])
util.write_wavfile(eventname+"_L1_whitenbp.wav",int(fs), strain_L1_whitenbp[indxd])

# re-whiten the template using the smoothed PSD; it sounds better!
template_p_smooth = util.whiten(template_p,psd_smooth,dt)
...
util.write_wavfile(eventname+"_template_whiten.wav",int(fs), template_p_smooth[indxt])
...
strain_H1_shifted = util.reqshift(strain_H1_whitenbp,fshift=fshift,sample_rate=fs)
strain_L1_shifted = util.reqshift(strain_L1_whitenbp,fshift=fshift,sample_rate=fs)

# write the files:
util.write_wavfile(eventname+"_H1_shifted.wav",int(fs), strain_H1_shifted[indxd])
util.write_wavfile(eventname+"_L1_shifted.wav",int(fs), strain_L1_shifted[indxd])

# and the template:
template_p_shifted = util.reqshift(template_p_smooth,fshift=fshift,sample_rate=fs)
util.write_wavfile(eventname+"_template_shifted.wav",int(fs), template_p_shifted[indxt])
